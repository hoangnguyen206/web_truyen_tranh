// public/assets/js/pages/latest.js
...
export async function render(query) {
    ...
    try {
        const data = await api.latest(currentPage);
        ...
        <main class="pt-20 pb-24 md:pb-8 px-4 md:px-6 max-w-7xl mx-auto" style="font-family: 'Be Vietnam Pro', sans-serif;">
            <header class="mb-8">
                <h1 class="text-2xl md:text-3xl font-black text-zinc-900 dark:text-white" style="font-family: 'Epilogue', sans-serif;">${t('latest_releases')}</h1>
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1">${t('page')} ${currentPage} / ${totalPages}</p>
            </header>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-4">
                ${items.map(m => latestCard(m, cdn)).join('')}
            </div>
            <div class="mt-8">
                ${renderLatestPagination(currentPage, totalPages)}
            </div>
        </main>`;
    } catch (e) {
...
function latestCard(manga, cdn) {
    ...
    <article class="group cursor-pointer" onclick="window.router.navigate('/manga/${manga.slug}')">
        <div class="relative aspect-[2/3] rounded-xl overflow-hidden mb-2 bg-zinc-200 dark:bg-zinc-800 shadow-sm group-hover:shadow-md transition-shadow">
            ...
            ${ch ? `<div class="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent">
                <span class="text-white text-[11px] font-bold">Ch. ${ch}</span>
            </div>` : ''}
        </div>
...
function renderLatestPagination(current, total) {
    ...
    let buttons = [];
    buttons.push(`<button class="pagination-btn" onclick="window._loadLatestPage(${current - 1})" ${current <= 1 ? 'disabled style="opacity:0.3;pointer-events:none"' : ''}>&laquo;</button>`);
    
    if (current > 3) {
        buttons.push(`<button class="pagination-btn" onclick="window._loadLatestPage(1)">1</button>`);
        if (current > 4) buttons.push(`<span class="px-1 text-zinc-400">...</span>`);
    }
    
    for (let i = Math.max(1, current - 2); i <= Math.min(total, current + 2); i++) {
        buttons.push(`<button class="pagination-btn ${i === current ? 'active' : ''}" onclick="window._loadLatestPage(${i})">${i}</button>`);
    }
    
    if (current < total - 2) {
        if (current < total - 3) buttons.push(`<span class="px-1 text-zinc-400">...</span>`);
        buttons.push(`<button class="pagination-btn" onclick="window._loadLatestPage(${total})">${total}</button>`);
    }
    
    buttons.push(`<button class="pagination-btn" onclick="window._loadLatestPage(${current + 1})" ${current >= total ? 'disabled style="opacity:0.3;pointer-events:none"' : ''}>&raquo;</button>`);
    
    window._loadLatestPage = (page) => {
        if (page < 1 || page > total) return;
        window.router.navigate(`/latest?page=${page}`);
    };
